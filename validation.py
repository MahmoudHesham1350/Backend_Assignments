...
class Validation:
    ...
    @staticmethod
    def get_valid_string(message: str) -> str:
        """
        Prompts the user to enter a valid string.

        Args:
            message (str): The message to display when prompting the user for input.

        Returns:
            str: The user's valid string input.
        """
        string = input(message)

        if not string:
            print("Cannot be empty")
            return Validation.get_valid_string(message)
        else:
            return string
